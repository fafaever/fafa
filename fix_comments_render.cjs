...
const search = `{[...selectedPost.comments]
                .reverse()
                .filter(c => {
                  if (!showOpOnly) return true;
                  // Show only comments made by thread OP or explicitly marked as isOpUpdate
                  return c.authorId === selectedPost.authorId || c.isOpUpdate;
                })
                .map(c => {
                if (c.isRecalled) {
                  return (
                ...
                return (
                  <div 
                    ...
                    className={\`bg-white rounded-xl p-3 shadow-xs border flex gap-3 relative transition-all \${
                      isUserComment 
...
const insert = `{(() => {
                ...
                sorted.forEach(c => {
                  ...
                  // If it has a replyTo, try to attach to parent. If parent doesn't exist, treat as top level.
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 space-y-2.5 min-w-0">
                              ...
                              {/* Only show the reply bar if the comment didn't actually get nested. Usually we don't need it if nested, but keeping it as context is fine */}
                              {c.replyTo && c.level === 0 && (
                                <div className="bg-neutral-50 border-l-2 border-neutral-300 px-2 py-1 rounded text-[11px] text-neutral-500 flex items-center gap-1 my-1">
                                  <CornerDownRight className="w-3 h-3 text-neutral-400 shrink-0" />
                                  <span>回复 #{c.replyTo.floor} @{c.replyTo.authorName}: {c.replyTo.content}</span>
                      ...
                      {c.children.length > 0 && (
                        <div className="mb-3">
                          <button
                            ...
                            className="text-[11px] font-bold text-neutral-500 hover:text-black transition-colors flex items-center gap-1 mb-2"
                          >
                            <span className="w-4 h-0.5 bg-neutral-300 rounded inline-block" />
...
if (code.includes(search)) {
  ...
  console.log("Comments rendered properly.");
} else {
  console.log("Could not find comments map block.");
}
