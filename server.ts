...
// Initialize default Gemini API client if key exists
...
app.post("/api/test-connection", async (req, res) => {
  ...
  try {
    // If testing custom connection
    if (apiUrl && apiKey) {
      const isGeminiUrl = apiUrl.includes("generativelanguage") || apiUrl.includes("google");
      
      if (isGeminiUrl) {
        // Direct test call to Google Gemini endpoint
        ...
        return res.json({ success: true, message: "Successfully connected to custom Gemini API!" });
      } else {
        ...
        if (!response.ok) {
          ...
          return res.status(response.status).json({
            success: false,
            message: `Custom API error (${response.status}): ${JSON.stringify(errData)}`,
          });
        }

        return res.json({ success: true, message: "Successfully connected to OpenAI-compatible endpoint!" });
      }
    }

    // Default connection test using server-side Gemini Key
    if (!ai) {
      return res.status(400).json({
        success: false,
        message: "Server-side GEMINI_API_KEY is not configured. Please provide a custom URL and Key in Settings.",
      });
    }
    ...
    res.json({ success: true, message: "Successfully connected to default Gemini API!" });
  } catch (error: any) {
    console.error("Connection test failed:", error);
    res.status(500).json({ success: false, message: error.message || "Unknown error during test" });
  }
});

// API endpoint to fetch list of available models from custom endpoint
app.post("/api/fetch-models", async (req, res) => {
  ...
  if (!apiUrl || !apiKey) {
    return res.status(400).json({
      success: false,
      message: "请先填写 API 接口地址和 API 密钥再试。",
    });
  }
  ...
  try {
    ...
    if (isGeminiUrl) {
      ...
      const fetchUrl = cleanUrl.includes("/models") 
        ? `${cleanUrl}?key=${apiKey}` 
        : `${cleanUrl}/models?key=${apiKey}`;

      const response = await fetch(fetchUrl);
      if (!response.ok) {
        const errText = await response.text();
        throw new Error(`Gemini API 返回错误: ${response.status} - ${errText}`);
      }
      ...
      if (data && Array.isArray(data.models)) {
        ...
        return res.json({ success: true, models: ["gemini-2.0-flash-lite", "gemini-2.5-flash"] });
      }
    } else {
      const cleanUrl = apiUrl.replace(/\/+$/, "");
      const fetchUrl = `${cleanUrl}/models`;
      ...
      if (data && Array.isArray(data.data)) {
        ...
        const modelNames = data.map((m: any) => typeof m === "string" ? m : m.id || m.name).filter(Boolean);
        return res.json({ success: true, models: modelNames });
      } else {
        throw new Error("无法从自定义接口返回结果中解析模型列表。");
      }
    }
  } catch (error: any) {
    console.error("Fetch models failed:", error);
    res.status(500).json({ success: false, message: error.message || "获取模型列表失败" });
  }
});

// Helper to strip any action descriptions (asterisks, parentheses, brackets) for Online Chat Mode
function cleanActionDescriptions(text: string): string {
  if (!text) return text;
  let cleaned = text;
  // Remove asterisks action blocks e.g. *smile*, *微微一笑*
  cleaned = cleaned.replace(/\*[^*]+?\*/g, "");
  // Remove double asterisks actions
  cleaned = cleaned.replace(/\*\*[^*]+?\*\*/g, "");
  // Remove parenthesis action blocks e.g. (叹气), （无奈）
  cleaned = cleaned.replace(/\([^)]+?\)/g, "");
  cleaned = cleaned.replace(/（[^）]+?）/g, "");
  // Remove square bracket action blocks e.g. [开心], 【点头】
  cleaned = cleaned.replace(/\[[^\]]+?\]/g, "");
  cleaned = cleaned.replace(/【[^】]+?】/g, "");
  
  // Clean up any left-over double spaces
  cleaned = cleaned.replace(/ {2,}/g, " ");
  // Clean up duplicate punctuation sequences (e.g. "，。" or "。，") caused by deletions
  cleaned = cleaned.replace(/[，。]{2,}/g, "。");
  cleaned = cleaned.replace(/^[，。、；：,.;:]+/g, ""); // strip leading punctuation
  
  return cleaned.trim();
}

// Dynamically parses and structures the character's core persona guidelines
...
// Check if a response violates any persona guidelines
function checkPersonaViolations(text: string, osText: string, character: any, parsedInfo: any) {
  const violations: string[] = [];
  
  if (!text || text.trim() === "") {
    violations.push("回复内容为空，可能被系统安全拦截过滤，或者生成失败。");
  }
  ...
  // 1. High-cold character checks
  if (isCold && text && text.trim() !== "") {
    // Exclamation marks check: "禁止用感叹号"
    if (text.includes("!") || text.includes("！")) {
      violations.push("检测到感叹号（！）。高冷冷静克制人设【严禁】使用感叹号！");
    }
    // Overly enthusiastic words check
    const enthusiasticWords = ["太好了", "开心", "太棒了", "真好", "哈哈", "嘻嘻", "啦啦", "撒娇", "么么", "爱你", "宝贝", "亲爱的"];
    for (const w of enthusiasticWords) {
      if (text.includes(w)) {
        violations.push(`检测到过于热情或肉麻的词汇: "${w}"。高冷人设应保持克制冷静。`);
        break;
      }
    }
    // Length check for cold response (should not be extremely talkative unless specifically requested)
    const sentences = text.split(/[，。！、？\n]/).map(s => s.trim()).filter(Boolean);
    if (sentences.length > 5) {
      violations.push("回复包含过多句子，显得太话唠。高冷人设回复应该极其简练、短小克制。");
    }
  }

  // 2. Warm character checks
  if (isWarm && text && text.trim() !== "") {
    const coldOrRudeWords = ["滚", "烦人", "讨厌你", "别烦我", "没空", "无聊", "随便你吧", "懒得理你"];
    for (const w of coldOrRudeWords) {
      if (text.includes(w)) {
        violations.push(`检测到过于粗鲁或冰冷抗拒的词汇: "${w}"。温柔可爱人设应该保持温柔、亲和。`);
        break;
      }
    }
  }

  // 3. Command tone check if forbidden
  const hasForbiddenCommand = (character.systemInstruction || "").includes("禁止命令") || (character.systemInstruction || "").includes("禁止命令式");
  if (hasForbiddenCommand && text && text.trim() !== "") {
    const commandWords = ["必须", "给我去", "去把", "立刻", "给我"];
    for (const w of commandWords) {
      if (text.includes(w)) {
        violations.push(`使用了禁止的命令式词汇: "${w}"。`);
        break;
      }
    }
  }

  // 4. Missing OS check (if required)
  if (!osText) {
    violations.push("未能成功在回复末尾附带格式正确的内心心声 [OS_INNER] (os：...) [情绪]。");
  }

  // 5. Check for banned AI/Refusal phrases (SYSTEM ERRORS)
  ...
  const combinedText = ((text || "") + " " + (osText || "")).toLowerCase();
  for (const phrase of bannedAIPhrases) {
    if (combinedText.includes(phrase)) {
      violations.push(`检测到包含系统禁止的AI/机器人用语或拒绝辞令: "${phrase}"。请你完全扮演真实的沉浸式角色，绝对不要说任何类似AI、人工智能、免责声明、或者由于道德/政策等原因而表示‘我无法’、‘我不能’、‘作为一个AI’等拒绝扮演或破坏沉浸感的话！请用角色人设对应的口吻巧妙规避或趣味性回答。`);
      break;
    }
  }
...
// Hard safety sanitizer and rewrite if final output still contains banned AI phrases
...
// Helper to extract the character's inner thoughts (OS)
function extractOS(rawText: string, mood?: string): { cleanText: string; os: string } {
  let cleanText = rawText;
  let os = "";

  // Try matching [OS_INNER] marker first
  const markerIndex = cleanText.indexOf("[OS_INNER]");
  if (markerIndex !== -1) {
    os = cleanText.substring(markerIndex + 10).trim();
    cleanText = cleanText.substring(0, markerIndex).trim();
  } else {
    // Try matching the pattern (os:...) [...] directly
    const osPatternRegex = /(（os：[\s\S]*?\)\s*\[[^\]]+\])/i;
    const match = cleanText.match(osPatternRegex);
    if (match) {
      os = match[1].trim();
  ...
  // Remove any system default text or placeholders that might have leaked
  if (os && os.includes("接下来该怎么接话呢")) {
    os = "";
  }

  // Clean up any double spaces, trailing line breaks or leftover commas
  cleanText = cleanText.trim();
  
  return { cleanText, os };
}

// Helper to safely parse JSON or extract it from markdown/text blocks
...
// Endpoint to parse character setting files (.docx / .txt) and extract settings
app.post("/api/analyze-character-file", async (req, res) => {
  const { fileBase64, fileName, settings } = req.body;
  
  if (!fileBase64 || !fileName) {
    return res.status(400).json({ success: false, error: "缺少必要的文件内容。 (Missing file content)" });
  }
  ...
  try {
    ...
    if (fileName.toLowerCase().endsWith(".txt")) {
      ...
      return res.status(400).json({ success: false, error: "仅支持 .txt 或 .docx 格式的文本文件。 (Only .txt or .docx are supported)" });
    }

    if (!text.trim()) {
      return res.status(400).json({ success: false, error: "无法从导入的文件中提取文本内容，或该文件为空。 (File text extraction failed or empty)" });
    }
...
2. 绝对不能有 markdown 代码块包裹（即绝对不要写 \`\`\`json 这样的头部，也不要有任何前导或后继的闲聊文字）。`;
    ...
    // 1. If using custom settings proxy
    if (customApiUrl && customApiKey) {
      const isGeminiUrl = customApiUrl.includes("generativelanguage") || customApiUrl.includes("google");
      if (isGeminiUrl) {
        ...
        const response = await customAi.models.generateContent({
          model: customModel || "gemini-2.0-flash-lite",
          contents: prompt,
        });
        responseText = response.text || "";
      } else {
        const response = await fetch(`${customApiUrl.replace(/\/+$/, "")}/chat/completions`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        if (!response.ok) {
          const errData = await response.json().catch(() => ({}));
          throw new Error(`Custom model API error (${response.status}): ${JSON.stringify(errData)}`);
        }
        const data = await response.json();
        responseText = data.choices?.[0]?.message?.content || "";
      }
    } else {
      // 2. Default Built-in Gemini
      if (!ai) {
        throw new Error("内置免费通道暂未配置或可用。请在设置中配置个人 API。");
      }
      const response = await ai.models.generateContent({
        model: "gemini-2.0-flash-lite",
        contents: prompt,
    ...
    parsedData.personality = text; // Pour original file content in verbatim without summarizing
    res.json({ success: true, data: parsedData });
  } catch (error: any) {
    console.error("Document analysis endpoint failed:", error);
    res.status(500).json({ success: false, error: error.message || "文档分析失败，请检查文件格式或重试。" });
  }
});

// Main chat API endpoint
app.post("/api/chat", async (req, res) => {
  console.log('后端收到的请求体:', req.body);
  const { messages, character, settings, matchedLore, chatMode, replyLength, replyCount, mood, memories, userDidNotReply, isBlocked } = req.body;
...
Answer in the character's voice. Stay strictly in character. Do not break character.`;
    ...
    while (attempt <= maxAttempts) {
      ...
      try {
        if (customApiUrl && customApiKey) {
          ...
          if (isGeminiUrl) {
            ...
            const response = await customAi.models.generateContent({
              model: customModel || "gemini-2.0-flash-lite",
              contents: formatInputMessages(messages),
              config: {
                systemInstruction: currentSysInstruction,
                temperature: 0.8,
              },
            });
            ...
            const formattedMessages = [
              ...
              ...messages.map((m: any) => ({
                ...
                content: m.isRecalled 
                  ? `[SYSTEM NOTE: The user has recalled this message. You saw this message, but you MUST pretend you didn't see it and act completely oblivious to its contents in your response.] ${m.content}`
                  : m.content,
              })),
            ];
            const response = await fetch(`${customApiUrl.replace(/\/+$/, "")}/chat/completions`, {
          ...
          // Default server-side Gemini API
          if (!ai) {
            return res.status(400).json({
              error: "默认服务端 GEMINI_API_KEY 未配置，且未在“我的 - 设置”中配置自定义 API 地址和密钥。请在“我的 - 设置”中进行配置以开始聊天。",
            });
          }
          const response = await ai.models.generateContent({
            model: "gemini-2.0-flash-lite",
        ...
        console.error(`Generation attempt ${attempt} failed with error:`, err);
        if (attempt === maxAttempts) {
          console.warn("[Persona Check Loop] All generation attempts failed or blocked. Generating immersive fallback deflection.");
          const fallbackResponse = sanitizeBannedPhrases("", "", character, parsedInfo);
          finalCleanText = fallbackResponse.cleanText;
          finalOs = fallbackResponse.osText;
          break;
        }
      ...
      console.log(`[Persona Check Loop] Attempt ${attempt} raw response:`, rawText);
      const { cleanText, os } = extractOS(rawText, mood);
      const processedText = isOnlineMode ? cleanActionDescriptions(cleanText) : cleanText;

      // Run consistency check!
      const violations = checkPersonaViolations(processedText, os, character, parsedInfo);

      if (violations.length === 0) {
        console.log(`[Persona Check Loop] Consistency validation PASSED on attempt ${attempt}.`);
        finalCleanText = processedText;
        finalOs = os;
        break;
      } else {
        console.warn(`[Persona Check Loop] Consistency validation FAILED on attempt ${attempt}. Violations:`, violations);
        if (attempt >= maxAttempts) {
          console.warn(`[Persona Check Loop] Max attempts reached. Falling back to last generated response.`);
          finalCleanText = processedText;
          finalOs = os;
          break;
        }

        // Refine systemInstruction for self-correction in the next attempt
        currentSysInstruction = `${sysInstruction}

========================================
⚠️ 【CRITICAL RE-GENERATION FEEDBACK / 人设一致性自我修正反馈】
你在上一次尝试中生成的回复未通过一致性检查。存在以下严重违规行为：
${violations.map((v, i) => `${i + 1}. ${v}`).join("\n")}

请认真阅读你的锚定人设和禁止行为。
现在，请【重新生成】回复。你必须：
- 严格遵循你的性格核心和说话风格，时刻维持人设，绝对禁止说出不符合该设定的不当词汇。
- 消除上述所有违规行为（例如：如果人设高冷，千万不要使用感叹号！）。
- 保证内心心声 [OS_INNER] 格式（（os：内心想法） [情绪标签]）正确且附在最末尾。
========================================
`;
        attempt++;
      }
    ...
    // Apply the Hard Safety Sanitizer and fallback deflection to guarantee compliance
    const sanitized = sanitizeBannedPhrases(finalCleanText, finalOs, character, parsedInfo);
    finalCleanText = sanitized.cleanText;
    finalOs = sanitized.osText;

    res.json({ text: finalCleanText, os: finalOs });
  } catch (outerError: any) {
    console.error("Chat generation critical failure:", outerError);
    res.status(500).json({ error: outerError.message || "后端服务器发生未知错误" });
  }
});
  ...
  try {
    ...
    if (customApiUrl && customApiKey) {
      ...
      if (isGeminiUrl) {
        ...
        // Here we could handle openai-compatible endpoint if we wanted, but let's just stick to Gemini if it's the only one supported easily for this endpoint, or we can use the fetch approach. Let's just use GoogleGenAI and assume they use Gemini. For openai compatible, we might need to fallback to fetch.
        // For simplicity, let's just use fetch for openai compatible
        const response = await fetch(`${customApiUrl.replace(/\/+$/, "")}/chat/completions`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        if (!response.ok) {
          throw new Error("Custom OpenAI API failed");
        }

        const data = await response.json();
        const text = data.choices?.[0]?.message?.content || "无法生成随笔。";
    ...
    const text = response.text || "无法生成随笔。";
    res.json({ text: text.trim() });
  } catch (err: any) {
    console.error("Note generation failed:", err);
    res.status(500).json({ error: err.message || "随笔生成失败" });
  }
});

// UNO Game dialogue generation endpoint
app.post("/api/uno-dialogue", async (req, res) => {
  const { character, event, cardDetails, context, settings } = req.body;
    ...
    if (customApiUrl && customApiKey) {
      ...
      if (isGeminiUrl) {
        try {
          ...
          console.error("UNO custom Gemini generation failed:", err);
          throw err;
        }
      } else {
        try {
          ...
          console.error("UNO custom OpenAI API generation failed:", err);
          throw err;
        }
      }
    } else {
      if (!ai) {
        throw new Error("Default server-side GEMINI_API_KEY is not configured.");
      }
      ...
      try {
        ...
        console.error("UNO default Gemini generation failed:", err);
        throw err;
      }
    }

    // Clean up response if it wraps inside quotes or brackets
    let cleaned = responseText.trim().replace(/^["'「『(（*]*/, "").replace(/["'」』)）*]*$/, "");

    // Consistency micro-adjustments for cold characters (such as removing exclamations if high-cold)
    const isCold = /冷|克制|静|高冷|傲娇|漠|毒舌|淡/i.test(parsedInfo.personality + " " + parsedInfo.chatStyle + " " + character.description + " " + character.name);
    if (isCold) {
      cleaned = cleaned.replace(/!/g, ".").replace(/！/g, "。");
    }

    res.json({ dialogue: cleaned });
  } catch (error: any) {
    console.error("UNO Game dialogue generation failed:", error);
    // Fallback template dialogues in case of failure
    let fallback = "到我了吗？看我的厉害！";
    if (event === "START") fallback = "UNO 局开打！大家准备好了吗？";
    else if (event === "DRAW_CARD") fallback = "唔，没牌出了，只能去摸一张……";
    else if (event === "SKIPPED") fallback = "诶？我的回合被跳过了吗？！";
    else if (event === "TARGETED_DRAW") fallback = "可恶，为什么要塞给我这么多牌！";
    else if (event === "UNO") fallback = "UNO！我快要赢啦！";
    else if (event === "WIN") fallback = "好耶！我赢啦，你们都太慢了！";
    else if (event === "LOSE") fallback = "呜呜，我居然输了……再来一局！";
    
    res.json({ dialogue: fallback, isFallback: true, error: error.message });
  }
});

// UNO Game AI decision-making endpoint
app.post("/api/uno-move", async (req, res) => {
  const { character, playableCards, topCard, currentColor, context, settings } = req.body;

  if (!playableCards || playableCards.length === 0) {
    return res.json({ cardId: null, chosenColor: null, dialogue: "没有能出的牌，摸一张看看吧。" });
  }
...
`;
  ...
  try {
    ...
    res.json({
      cardId: parsed.cardId || playableCards[0]?.id || null,
      chosenColor: parsed.chosenColor || null,
      dialogue: parsed.dialogue || "看我的，这一张！"
    });
  } catch (error: any) {
    console.error("UNO AI move decision API failed:", error);
    // Fallback: pick the first playable card
    const fallbackCard = playableCards[0];
    res.json({
      cardId: fallbackCard?.id || null,
      chosenColor: fallbackCard?.color === "wild" ? "red" : null,
      dialogue: "该我了，接招吧！",
      isFallback: true
    });
  }
...
// Generate Turtle Soup Batch endpoint
app.post("/api/generate-turtlesoup-batch", async (req, res) => {
...
[
  ...
  try {
    ...
    if (customApiUrl && customApiKey) {
      ...
      if (!ai) {
        throw new Error("默认服务端 GEMINI_API_KEY 未配置，请在设置中配置自定义 API 地址和密钥。");
      }
    ...
    if (Array.isArray(parsed)) {
      res.json({ success: true, puzzles: parsed });
    } else {
      throw new Error("AI 返回的格式不是合法的 JSON 数组");
    }
  } catch (error: any) {
    console.error("Generate Turtle Soup Batch API failed:", error);
    res.status(500).json({ success: false, error: error.message || "生成海龟汤题目失败" });
  }
});

// Error handling middleware for API routes to always return JSON errors
